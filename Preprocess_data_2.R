...
d <- read.table(base_path+"pancreas_refseq_rpkms_counts_3514sc.txt", stringsAsFactors = F)
d <- d[!duplicated(d[,1]), ]
rownames(d) <- d[,1]
d <- d[,3:ncol(d)]
d <- d[,3515:7028]
labs <- read.table(base_path+"labels.txt", stringsAsFactors = F, header = F)
...
### ANNOTATIONS
ann <- read.table(base_path+"E-MTAB-5061.sdrf.txt", stringsAsFactors = F, header = T, sep = "\t")
...
saveRDS(sceset, save_path+"target.rds")
